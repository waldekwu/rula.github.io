 ...
 */
 ...
 let upperArmValueL = 0;
 let upperArmAdjValueL = 0;

 let lowerArmValueL = 0;
 let lowerArmAdjValueL = 0;

 let wristValueL = 0; 
 let wristAdjValueL = 0;

 let wristTwistValueL = 0;
...
function getInput() {
	...
	switch (currentQuestionIndex) {
		...
		if (document.querySelector('input[name="customCheck-q5"]:checked')) {
			...
			for (var i = 0; i < options.length; i++) {
				if (options[i].type === 'checkbox' && options[i].checked === true) {
					upperArmAdjValueL += parseInt(options[i].value);
					//summary
					document.getElementById("sum-"+options[i].id).classList.remove("sum-li");
				}
...
function setTableScores() {
	console.log('You scored: ' + (upperArmValue + upperArmAdjValue).toString() + 
    ...
    console.log('You scored: ' + (upperArmValueL + upperArmAdjValueL).toString() + 
    	(lowerArmValueL + lowerArmAdjValueL).toString() + 
    	(wristValueL + wristAdjValueL).toString() +
    	wristTwistValueL.toString())
    
    totalAValueL = (upperArmValueL + upperArmAdjValueL).toString() + 
    (lowerArmValueL + lowerArmAdjValueL).toString() + 
    (wristValueL + wristAdjValueL).toString() +
    wristTwistValueL.toString();

    AScoreL = parseInt(tables[0][totalAValueL]);
...
function setOutput() {
	...
	//left
	if (AScoreL !== AScoreL) {
		$("part-a-score-l").html('0');

	} else {
		wristArmScoreL = AScoreL + parseInt(forceLoadValue + muscleUseValue);

		$("#posture-a-l").html(parseInt(AScoreL));
		$("#part-a-score-l").html(wristArmScoreL);
	}
	...
	if (BScore !== BScore) {
		...
		let tableCScoreL = wristArmScoreL + "x" + neckTrunkLegsScore;
		finalScoreL = parseInt(tables[2][tableCScoreL]);

	}
	console.log("This is final score: " + finalScore + "This is final left score: " + finalScoreL);
}
...
function rulaScore() {
	...
	if (finalScore < 3) {
        ...
        $('#score-container, #rula-scorecard').html(
        	...
        	<h5>RULA Score (Right): ${finalScore}</h5>
...
if (finalScoreL < 3) {
        ...
        $('#score-container-l, #rula-scorecard-l').html(
        	...
        	<ul style="list-style: none; padding: 15px;">
        	<li style="color: white;">
        	<h5>RULA Score (Left): ${finalScoreL}</h5>
        	Action level 1: The posture is acceptable if it is not maintained or repeated for long periods
        	</li>
        	</ul>
        	</div>`);
        
    } else if (finalScoreL > 2 && finalScoreL < 5) {
        // resultsContainer.classList.add("f-investigation");
        // resultsImageContainer.innerHTML = "<img src='media/manikin_logo.png' class='card-img'>"
        $('#score-container-l, #rula-scorecard-l').html(
        	`<h4 style="text-align: left;"><img src="./media/icons/target.png" alt="Final RULA Score Icon" style="float: left; margin-right: 5px;">Left side:</h4>
    ...
    else if (finalScoreL > 4 && finalScoreL < 7) {
        // resultsContainer.classList.add("change-soon");
        // resultsImageContainer.innerHTML = "<img src='media/manikin_logo.png' class='card-img'>"
        $('#score-container-l, #rula-scorecard-l').html(
        	`<h4 style="text-align: left;"><img src="./media/icons/target.png" alt="Final RULA Score Icon" style="float: left; margin-right: 5px;">Left side:</h4>
...
//clone personal details
  ...
  switch (document.querySelector('input[name="radio-q1"]:checked').id) {
  	case "radio1-q1":
  	$("#sum-1-img").html(`<img src="./media/summary/sum-1/sum-1-1.jpg" alt="" height="180px">`);
  	break;

  	case "radio2-q1":
  	$("#sum-1-img").html(`<img src="./media/summary/sum-1/sum-1-2.jpg" alt="" height="180px">`);
  	break;

  	case "radio3-q1":
  	$("#sum-1-img").html(`<img src="./media/summary/sum-1/sum-1-3.jpg" alt="" height="180px">`);
  	break;

  	case "radio4-q1":
  	$("#sum-1-img").html(`<img src="./media/summary/sum-1/sum-1-4.jpg" alt="" height="180px">`);
  	break;

  	case "radio5-q1":
  	$("#sum-1-img").html(`<img src="./media/summary/sum-1/sum-1-5.jpg" alt="" height="180px">`);
  	break;
